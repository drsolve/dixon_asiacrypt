...
For these parameters the final merge~\eqref{eq:mitm_complexity} dominates every forward and backward stage of the chain, so it alone determines the reported complexity. The stage-by-stage figures and a rank bound that keeps the intermediate rungs below the merge are given in the full version~\cite{cryptoeprint:2026/1969}. This is the structured regime where local elimination is preferable to direct elimination.
