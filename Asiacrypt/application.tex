...
We consider the CICO-3 problems of \poseidon~\cite{DBLP:conf/uss/0001KR0S21} and \poseidonnew~\cite{DBLP:conf/africacrypt/GrassiKS23} in sponge mode, using the input/output and internal-subspace models from~\cite{DBLP:journals/tosc/GrassiKR25}; precise descriptions of the models and permutation are included in the full version~\cite{cryptoeprint:2026/1969}. For the pure I/O model, direct Dixon elimination matches the natural square-system viewpoint. For the mixed-degree subspace models, the Hessenberg bound from Section~\ref{Section:Complexity} becomes essential because the polynomial degrees are no longer uniform.
...
\caption{Complexity comparison for \poseidonnew, with the model parameters $t=8$, $c=d=3$, $\alpha=3$ of~\cite{DBLP:journals/tosc/GrassiKR25}. Here $\omega = 2.81$ for all curves.}
...
\textbf{Configuration} & \multicolumn{2}{c}{$d_{\mathrm{reg}}$} & \multicolumn{2}{c}{Comp.} & Time & \multicolumn{2}{c}{$D(\mathcal{F})$} & \multicolumn{2}{c}{Comp.} & Time \\
...
For \vision~\cite{DBLP:journals/tosc/AlyABDS20} with $s=2$, inverse layers make the full direct model unattractive, so we adopt a meet-in-the-middle (MITM) approach following the general framework of Liu et al.~\cite{DBLP:conf/asiacrypt/LiuMM24} and apply iterative Dixon elimination. The full version~\cite{cryptoeprint:2026/1969} gives the detailed model, equation distribution, and round function; Figure~\ref{fig:vision_complexity_comparison} and Table~\ref{tab:vision_timing} summarize the outcome here.
...
For these parameters the final merge~\eqref{eq:mitm_complexity} dominates every forward and backward stage of the chain, so it alone determines the overall complexity. The stage-by-stage figures and a rank bound that keeps the intermediate rungs below the merge are given in the full version~\cite{cryptoeprint:2026/1969}. The special structure of \vision yields observed degree growth far below the B\'ezout-based upper bounds. A more precise estimate is left for future work.
