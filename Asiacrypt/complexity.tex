...
\le\frac{(nd)^{n-1}}{n!},
\]
which also verifies the integer volume bound $B_{MH}$ in~\eqref{eq:Dixon_volume_bound}. Numerical comparisons and support-size experiments are reported in the full version~\cite{cryptoeprint:2026/1969}.
...
At the idealized boundary $\omega=2$, both stages must be retained. For $\omega>2$, choosing the best applicable model at each stage makes Step~4 asymptotically dominant in the fixed-$d$ or fixed-$n$ regimes analyzed in the full version~\cite{cryptoeprint:2026/1969}. It also dominates the running time in our experiments, so we use $T_{4}^{\mathrm{best}}$ as the asymptotic estimate.
