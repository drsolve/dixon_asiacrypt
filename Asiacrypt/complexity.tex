...
We bound the Dixon matrix size for general $(d_1,\ldots,d_n)$-systems via a lattice path count. The required background on polynomial supports, Newton polytopes, and lattice paths is reviewed in the full version~\cite{cryptoeprint:2026/1969}.
...
\begin{lemma}\label{lem:lattice_path_formulas}
	...
	\begin{enumerate}
		...
		\begin{equation}\label{eq:fuss_catalan}
...
\begin{corollary}\label{cor:three_bounds}
	...
	\begin{enumerate}
		...
		\end{equation}
		The bound is attained for generic systems by Theorem~\ref{thm:dixon_general_bound} and Lemma~\ref{lem:lattice_path_formulas}(2), under the characteristic condition stated in Remark~\ref{rem:support_vs_rank}.
		...
		\end{equation}
		The bound is attained for generic systems by Theorem~\ref{thm:dixon_general_bound} and Lemma~\ref{lem:lattice_path_formulas}(3), under the characteristic condition stated in Remark~\ref{rem:support_vs_rank}.
...
We use the Iterated-Simplex Polynomial to connect the Dixon matrix size to lattice path counting. Lemma~\ref{lem:fc_polytope} describes its support by linear inequalities. Lemma~\ref{lem:dixon_fc_bound} then bounds the Dixon matrix size by the monomial count of $I_{d_2-1,\ldots,d_n-1}$. Finally, Lemma~\ref{lem:lattice_path_count} counts these monomials via a bijection with lattice paths below a piecewise linear boundary.
...
A proof is given in the full version~\cite{cryptoeprint:2026/1969}.
...
Each coefficient just exhibited in $\Delta_{\mathcal F}(\mathbf1,\bar\xvec)$ is a polynomial in the input coefficients, nonzero at $\mathcal F^*$. The coefficient space over $\overline{\mathbb K}$ is an affine space and hence irreducible. The simultaneous nonvanishing of these finitely many coefficient polynomials defines a nonempty open set $U$, which is therefore dense; see~\cite[Chapter~3, \S5, Definition~(5.6), p.~115]{cox2005using}. Reversing the variable order permutes the input coefficients, so $U^{\mathrm{rev}}:=\{\mathcal F:\mathcal F^{\mathrm{rev}}\in U\}$ is also dense and open. On $U\cap U^{\mathrm{rev}}$, equation~\eqref{eq:tau_symmetry} ensures that $\Delta_{\mathcal F}$ has exactly $N^*$ distinct monomials in each of $\xvec$ and $\bar\xvec$. Thus the matrix is generically square of size $N^*$.
\qed
\end{proof}
...
\]
where $U$ and $R$ denote upward and rightward steps. This path has exactly $n$ horizontal steps, and its $k$-th horizontal step has height $\alpha_1+\cdots+\alpha_k\le a_k$. The final $\alpha_{n+1}$ upward steps bring it to $(n,a_n)$.
...
\paragraph{Proof of Theorem~\ref{thm:dixon_general_bound}.}
By Lemma~\ref{lem:dixon_fc_bound}, it remains to count the
monomials of $I_{d_2-1,\ldots,d_n-1}$.
Applying Lemma~\ref{lem:fc_polytope} in $n-1$ variables,
their exponent vectors satisfy
\[
\sum_{\ell=n-j}^{n-1}\alpha_\ell
\le \sum_{i=1}^{j}(d_{n-i+1}-1),
\qquad 1\le j<n.
\]
Reversing the coordinates via $\beta_i=\alpha_{n-i}$ turns
these inequalities into the prefix constraints
$\beta_1+\cdots+\beta_j\le a_j$.
By Lemma~\ref{lem:lattice_path_count}, the resulting
nonnegative lattice points correspond exactly to paths from
$(0,0)$ to $(n-1,a_{n-1})$ whose $j$-th horizontal step has
height at most $a_j$. This is precisely the boundary stated
in the theorem. Generic attainment follows from
Lemma~\ref{lem:dixon_fc_bound}.
\qed
...
\le\frac{(nd)^{n-1}}{n!},
\]
which also verifies the integer volume bound $B_{\mathrm{MH}}$ in~\eqref{eq:Dixon_volume_bound}. Numerical comparisons and support-size experiments are reported in the full version~\cite{cryptoeprint:2026/1969}.

\subsection{Complexity Estimates via Our Refined Bound}\label{Section:Estimate}

For clarity of exposition, we focus on $(n;d)$-systems with $n\ge2$, $d\ge2$, and $k\ge1$, where all polynomials have total degree $d$ in all variables. Consider $\mathcal{F} = \{f_1, \ldots, f_n\} \subset \mathbb{K}[\xvec,\avec]$ consisting of $n$ equations in $n-1+k$ variables, where $\xvec=(x_1,\ldots,x_{n-1})$ are the eliminated variables and $\avec$ the retained parameters. By Corollary~\ref{cor:three_bounds}(2), the generic Dixon matrix size is
\[
...
D_4 \le d^{\,n}.
$
Put $C=C_n^{(d)}$ and $\mu_4=\binom{nd+k}{k}$. Including matrix evaluation, the corresponding parameters are
\begin{equation}
 N_4\le(d^n+1)^k,\quad
 L_4=\widetilde O(C^\omega+C^2\mu_4),\quad
 \delta_4=nd(Cnd+1)^{k-1}.
 \label{eq:step4_params}
\end{equation}
where $\delta_4$ bounds the average column degree after Kronecker substitution and reduces to $\delta_4=nd$ when $k=1$. Therefore,
\begin{equation}
	T_{4}^{\mathrm{HNF}}
	=
...
\begin{equation}
	...
	=
	\widetilde{O}\!\left((d^{\,n}+1)^k\left(C^\omega+C^2\mu_4+k\,d^{\,n}\right)\right).
	\label{eq:step4_eval}
\end{equation}
...
At the idealized boundary $\omega=2$, both stages must be retained. For $\omega>2$ and fixed $k$, under the applicable field and degree hypotheses, Step~4 is asymptotically dominant among the displayed cost bounds in the fixed-$d$ or fixed-$n$ regimes analyzed in the full version~\cite{cryptoeprint:2026/1969}. It also dominates the running time in our experiments, so we use $T_{4}^{\mathrm{best}}$ as the asymptotic estimate.
...
With the best applicable construction model, Step~4 is asymptotically dominant for $\omega>2$ in the regimes analyzed in the full version~\cite{cryptoeprint:2026/1969}, and is also the empirical bottleneck. We therefore adopt $T=T_{4}^{\mathrm{HNF}}$ as the asymptotic estimate for well-determined systems; the same reference gives the model-specific comparisons.

\begin{remark}\label{rem:mixed_degree_complexity}
For mixed-degree systems, we bound $D(\mathcal F)$
using the Hessenberg Bound of Corollary~\ref{cor:three_bounds}(3)
and take the maximum cost over all stages.
In particular, for $k=1$ and total-degree bounds $d_1,\ldots,d_n$,
Step~4 costs $\widetilde{O}((\sum_i d_i)D(\mathcal F)^\omega)$
and dominates in most cases.
We omit a detailed mixed-degree analysis.
\end{remark}
...
\begin{table}[!htb]
	\centering
	\caption{Arithmetic cost bounds for well-determined $(n;d)$-systems, with $d_{\mathrm{reg}}=n(d-1)+1$, $d_{\mathcal I}=d^n$, and $t=\Theta(d^{n-1}/\sqrt n)$. Ratios omit logarithmic factors. The three displayed Gr\"obner-basis ratios (classical, F5, and rectangular) have the same orders in both standard asymptotic directions. The displayed FGLM and BNS ratios use $n$ fixed and $d\to\infty$. Complete derivations are given in the full version~\cite{cryptoeprint:2026/1969}.}
	...
	\begin{tabular}{lcc}
		...
		\midrule
		GB (classical)~\cite{Faugere2004} & $\displaystyle \mathcal{O}\left(\binom{n+d_{\mathrm{reg}}}{d_{\mathrm{reg}}}^{\omega}\right)$ & $\mathcal{O}\left((nd)^{\omega-1}\right)$ \\
		\midrule GB (F5)~\cite{DBLP:journals/jsc/BardetFS15,DBLP:journals/tosc/BariantBLP22} & $\displaystyle \mathcal{O}\left(n \cdot d_{\mathrm{reg}} \cdot \binom{n+d_{\mathrm{reg}}-1}{d_{\mathrm{reg}}}^{\omega}\right)$ & $\mathcal{O}\left(n^{\omega+1}\right)$ \\
		\midrule
		
		GB (rectangular)~\cite{DBLP:journals/jsc/BardetFS15,storjohann2000algorithms} & $\displaystyle \mathcal{O}\left(
		n \cdot \binom{n + d_{\mathrm{reg}} - d}{n} \cdot \binom{n + d_{\mathrm{reg}}}{n}^{\omega - 1}
		\right)$ & $\mathcal{O}\left(n^{\omega} d^{\omega-1}\right)$ \\
		\midrule
		GB (per-deg)~\cite{DBLP:phd/hal/Spaenlehauer12,DBLP:journals/tosc/KoschatkoLR24} & $\displaystyle \mathcal{O}\left(n \cdot \sum_{i=0}^{d_{\mathrm{reg}}} \binom{n+i-d-1}{i-d}\binom{n+i-1}{i}^{\omega-1} \right)$ &  \multicolumn{1}{c}{\textemdash}  \\
		\midrule
...
The Gr\"obner-based approach includes both the grevlex basis computation and the change to lexicographic order, so the relevant comparison is with the combined F4/F5$+$FGLM cost. We also include the HNF-based change-of-order algorithm of~\cite{BerthomieuNeigerSafey2022} as an alternative to FGLM. In both asymptotic regimes considered, the displayed Dixon bound is asymptotically smaller than the combined bound.
However, these Gr\"obner basis costs are upper bounds that do not fully capture practical F4/F5 optimizations. Finer estimates for F5~\cite{DBLP:journals/jsc/BardetFS15} and F4 Tracer~\cite{kouba2026complexityanalysisf4grobner} are available under additional assumptions and not included in our comparison. The ratios therefore compare general upper bounds, while Section~\ref{Implementation} provides practical comparisons. Figure~\ref{fig:complexity_comparison_d} evaluates the unsimplified cost expressions as $n$ varies, including the per-degree sum; further plots and ratio derivations are given in the full version~\cite{cryptoeprint:2026/1969}.
