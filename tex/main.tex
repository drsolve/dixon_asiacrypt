...
Against the closest Dixon software the gap is larger still: on dense $3$-variable systems of degree $8$ over $\Fp$, \drsolve\ terminates in about $0.1$~s where Dixon-EDF inside Fermat~\cite{fermat,lewis2015dixon} needs about $15$~s, while the Maple${+}$C code of Jinadu and Monagan~\cite{DBLP:conf/casc/JinaduM22,JinaduMonagan2025} targets parametric systems over $\mathbb{Q}$ and is not directly comparable here. Most of this speedup comes from the fast polynomial-matrix routines of FLINT/PML rather than from a change of elimination strategy.
