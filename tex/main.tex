...
Throughout this section, $D(\mathcal{F})$ denotes the support size of the Dixon matrix $\mathcal{M}$ associated with $\mathcal{F}$, that is, the maximum of the numbers of distinct $\xvec$-monomials and $\overline{\xvec}$-monomials occurring in the Dixon polynomial. We denote a system of $n$ polynomials with total-degree bounds $d_1, d_2, \ldots, d_n$ as a $(d_1,\ldots,d_n)$-system. The special case $d_1=\cdots=d_n=d$ is referred to as an $(n;d)$-system, following~\cite{DBLP:conf/stoc/KapurS96}.
...
\begin{theorem}[Lattice Path Bound]\label{thm:dixon_general_bound}
	...
	Moreover, if $\mathrm{char}(\mathbb{K})=0$ or $\mathrm{char}(\mathbb{K})>\sum_{i=2}^{n}(d_i-1)$, this bound is attained for generic polynomial systems.
\end{theorem}
...
\begin{lemma}\label{lem:reorder_support}
	Let $(d_1, d_2, \ldots, d_n)$ be a sequence with $d_1 \leq d_2 \leq \cdots \leq d_n$, and let $(d'_1, d'_2, \ldots, d'_n)$ be any permutation of this sequence. Then 
	$$\operatorname{Supp}(I_{d'_1, d'_2, \ldots, d'_n}) \subseteq \operatorname{Supp}(I_{d_1, d_2, \ldots, d_n}).$$
\end{lemma}

\begin{proof}
	By Lemma~\ref{lem:fc_polytope}, the support is determined by the inequalities
	$
	\alpha_{n-k+1} + \cdots + \alpha_n \le \sum_{j=1}^{k} d_{n-j+1}.
	$
	For any permutation $(d'_1,\dots,d'_n)$, the sum
	$
	\sum_{j=1}^{k} d'_{n-j+1}
	$
	is bounded above by the sum of the $k$ largest elements of the multiset $\{d_1,\dots,d_n\}$, which is precisely
	$
	\sum_{j=1}^{k} d_{n-j+1}
	$
	under the sorted ordering $d_1 \le \cdots \le d_n$.
	Hence the constraints defining $\mathrm{Supp}(I_{d'_1,\dots,d'_n})$ are no weaker than those for $\mathrm{Supp}(I_{d_1,\dots,d_n})$, implying the inclusion.
...
\begin{lemma}\label{lem:dixon_fc_bound}
	Let $\mathcal{F} = \{f_1, \ldots, f_n\} \subset \mathbb{K}[x_1, \ldots, x_{n-1}]$ be a system of polynomials with total degrees at most $d_1, d_2, \ldots, d_n$ respectively, where $d_1 \leq d_2 \leq \cdots \leq d_n$. Then $D(\mathcal{F})$ is bounded above by the number $N^{*}$ of monomials in the Iterated-Simplex Polynomial ${I}_{d_2-1, d_3-1, \ldots, d_{n}-1}$, and this bound is attained for generic polynomial systems provided $\mathrm{char}(\mathbb{K})=0$ or $\mathrm{char}(\mathbb{K})>\sum_{i=2}^{n}(d_i-1)$.
\end{lemma}

\begin{proof}
	After the cancellation matrix~\eqref{eq:Cancellation Matrix} is constructed, the Dixon polynomial admits the bilinear form $\Delta(\xvec,\bar\xvec)=X\mathcal{M}\overline{X}^{T}$ of Section~\ref{Section:Preliminaries}, so the number of $\bar\xvec$-monomials and the number of $\xvec$-monomials are the row and column dimensions of $\mathcal{M}$. We therefore count the monomials of $\Delta$ in $\bar\xvec$ by treating the original variables as generic constants $x_i=c_i$ for $1\le i<n$. The cancellation matrix becomes:
	\begin{equation}\notag
		C'(\xvec, \overline{\xvec}) = 
		\begin{bmatrix}
		...
		\end{bmatrix}.
		%\label{eq:Cancellation_Matrix}
	\end{equation}
	
	After applying the refined construction of the cancellation matrix from the Dixon polynomial definition, the leading determinantal expansion term is the product $\prod_{i=1}^{n} \widehat{C}_{i,i}$. The factor $\widehat{C}_{1,1}$ has $\bar\xvec$-degree $0$ and contributes no $\bar\xvec$-monomial; the remaining $n-1$ factors realize the Iterated-Simplex Polynomial ${I}_{d_2-1, d_3-1, \ldots, d_{n}-1}$ with degrees in sorted order $d_2-1 \leq d_3-1 \leq \cdots \leq d_{n}-1$. For any other permutation $\sigma$, the product $\prod_i \widehat{C}_{i,\sigma(i)}$ has its $\bar\xvec$-degree in row $i$ bounded by $d_{\sigma(i)}-1$, so its $\bar\xvec$-support satisfies the inequalities of Lemma~\ref{lem:fc_polytope} with the sequence $(d_{\sigma(2)}-1,\ldots,d_{\sigma(n)}-1)$. The multiset $\{d_{\sigma(2)},\ldots,d_{\sigma(n)}\}$ is obtained from $\{d_2,\ldots,d_n\}$ by replacing $d_{\sigma(1)}$ with $d_1\le d_{\sigma(1)}$, so every right-hand side in Lemma~\ref{lem:fc_polytope} is no larger than the corresponding one for a permutation of $(d_2,\ldots,d_n)$; combined with Lemma~\ref{lem:reorder_support}, this support is therefore contained in that of the sorted main-diagonal product, so the main-diagonal term carries the maximal monomial set. Therefore, the number of $\bar\xvec$-monomials of $\Delta$ is bounded above by the number of monomials in ${I}_{d_2-1, d_3-1, \ldots, d_{n}-1}$.
	
	The same bound holds for the number of $\xvec$-monomials of $\Delta$, by means of the order-reversing involution
	\begin{equation}\notag
		\tau:\quad x_i \longmapsto \bar{x}_{n-i},\qquad \bar{x}_i \longmapsto x_{n-i},\qquad 1\le i\le n-1 .
	\end{equation}
	Let $\mathcal{F}^{\mathrm{rev}}=\{f_1^{\mathrm{rev}},\ldots,f_n^{\mathrm{rev}}\}$ with $f_j^{\mathrm{rev}}(x_1,\ldots,x_{n-1}) := f_j(x_{n-1},\ldots,x_1)$; this system has the same degree sequence $(d_1,\ldots,d_n)$ as $\mathcal{F}$. A direct check shows that applying $\tau$ to the $i$-th row of the cancellation matrix~\eqref{eq:Cancellation Matrix} of $\mathcal{F}$ produces exactly the $(n-i+1)$-st row of the cancellation matrix of $\mathcal{F}^{\mathrm{rev}}$, so $\tau$ merely reverses the row order and preserves $\det(C)$ up to sign, while $\prod_{i=1}^{n-1}(x_i-\bar{x}_i)$ is multiplied by $(-1)^{n-1}$. Hence $\tau\bigl(\Delta_{\mathcal{F}}(\xvec,\bar\xvec)\bigr)=\pm\,\Delta_{\mathcal{F}^{\mathrm{rev}}}(\xvec,\bar\xvec)$, and $\tau$ carries $\xvec$-monomials to $\bar\xvec$-monomials bijectively. Applying the bound just proved to $\mathcal{F}^{\mathrm{rev}}$ therefore bounds the number of $\xvec$-monomials of $\Delta_{\mathcal{F}}$ by the same iterated-simplex count. Both row and column dimensions of $\mathcal{M}$ are therefore bounded above by the iterated-simplex monomial count, which is the claimed bound.
	
	%\textbf{Tightness for generic systems.}
	To establish that the bound is achieved for generic systems, we specialize the system to a nested form $\mathcal{F}^* = \{f_1, \ldots, f_n\}$ where $f_1 = 1$ and for $i=2, \ldots, n$:
	\begin{equation}\notag
		f_i = \left( \sum_{k=1}^{i-1} x_k \right)^{d_i}.
	\end{equation}
	Note that the iterated-simplex bound $I_{d_2-1,\ldots,d_n-1}$ does not depend on $d_1$, so taking $f_1$ to be a constant is consistent with the coefficient space of $(d_1,\ldots,d_n)$-systems used in the genericity argument below. Following the refined construction of the cancellation matrix $\widehat{C}$ (\ref{eq:Dixon_polynomial_improve}), the rows are defined via recursive difference quotients. For $j < i$, the polynomial $f_j$ depends only on $x_1,\ldots,x_{j-1}$, so it does not contain $x_{i-1}$; hence $C_{i,j} = C_{i-1,j}$ and the numerator of the difference quotient in row $i$ vanishes identically, giving $\widehat{C}_{i,j} = 0$ for $j < i$. Consequently, $\widehat{C}$ is upper-triangular:
	
	\begin{equation}\notag
		\widehat{C} = \begin{bmatrix}
			1 & f_2 & f_3 & \cdots & f_n \\
		...
		\end{bmatrix}
	\end{equation}
	where the diagonal entries $M_{i,i}$ ($i=2, \ldots, n$) are given by:
	\begin{equation}\notag
	...
	Each $M_{i,i}$ is (a constant multiple of) a complete homogeneous symmetric form in the two arguments $\bar{X}_{i-2}+x_{i-1}$ and $\bar{X}_{i-2}+\bar{x}_{i-1}$. After specializing $x_{i-1}=c_{i-1}$ for a generic constant $c_{i-1}$, every term in the resulting expansion is a non-negative multinomial coefficient times a monomial in $\bar{x}_1,\ldots,\bar{x}_{i-1}$, so no coefficient cancellation occurs. Thus, the support of $M_{i,i}$ with respect to $\bar{x}_1, \ldots, \bar{x}_{i-1}$ is exactly a simplex of degree $d_i-1$. 
	The Dixon polynomial is obtained as $\Delta(\xvec, \bar{\xvec}) = \det(\widehat{C}) = \prod_{i=2}^{n} M_{i,i}$. The support of $\Delta$ is precisely characterized by the Iterated-Simplex Polynomial $I_{d_2-1, \ldots, d_{n}-1}$. 
	
	Attainment for generic systems follows from a Zariski-density argument. For $\mathcal{F}^*$ the number of $\bar\xvec$-monomials of $\Delta$ equals the iterated-simplex monomial count $N^*$, which by the first part of the proof is the generic maximum; since $D(\mathcal{F})$ is by definition the maximum of the two support sizes, this is all that is required (the $\xvec$-support of $\mathcal{F}^*$ may well be strictly smaller). The condition that every iterated-simplex monomial appears in $\Delta$ with a nonzero coefficient is a finite conjunction of nonvanishing polynomial conditions on the coefficients of $\mathcal{F}$, all satisfied at $\mathcal{F}^*$; it therefore defines a non-empty, hence dense, Zariski-open subset of the irreducible coefficient space, on which the $\bar\xvec$-support of $\Delta$ has size $N^*$. Hence $D(\mathcal{F}) = N^*$ is attained generically. All multinomial coefficients arising in the relevant expansions have upper index at most $\sum_{i=2}^{n}(d_i-1)$, so the stated characteristic assumption prevents their vanishing modulo $\operatorname{char}(\mathbb{K})$.
	
\end{proof}

\begin{remark}\label{rem:support_vs_rank}
	Throughout the matrix-size discussion, ``generic'' is understood in the usual Zariski-open sense of Definition~\ref{def:generic}, and $D(\mathcal{F})$ denotes the row/column dimension of $\mathcal{M}$, i.e.\ the size of its monomial support. This is an upper bound for, and in general strictly larger than, $\operatorname{rank}\mathcal{M}$: exhibiting all row and column monomials does not exhibit a non-vanishing minor of size $N^{*}$. Support containment, and hence the upper bound, holds over any field; only attainment uses the hypothesis on the characteristic. Over finite fields the formulas should be read as upper-bound templates after specialization rather than as exact equalities for every structured instance.
\end{remark}
...
\end{equation}
where $\mu_1$ is the dense monomial count of one polynomial of total degree $d$ in $n-1+k$ variables, the factor $M^2$ in $S_1$ comes from the Dixon bilinear support and $\binom{nd+k}{k}$ from the parameter monomials of total degree at most $B_1=nd$, and the constant-determinant cost $n^\omega$ inside $L_1$ is absorbed into the $n^2\mu_1$ matrix-evaluation term since $\mu_1\ge n-1+k$. Substituting~\eqref{eq:step1_params} into the two preferred determinant models yields
\begin{equation}
...
\begin{hypothesis}[Degree Minimization Heuristic]\label{hyp:deg_mini_hypo}
	...
	$\min(|\text{rdeg}(A)|, |\text{cdeg}(A)|) > \min(|\text{rdeg}(B)|, |\text{cdeg}(B)|),$
	then we expect that $\deg(\det(A)) > \deg(\det(B))$ holds in the generic case.
\end{hypothesis}
...
\begin{proof}
	...
	Assume $n \geq 2$ and $P_1 + \cdots + P_{n-1} = Q_{n-1}$ (viewing $Q_{n-1}$ as a subset of $\mathbb{Z}_{\geq 0}^{n-1} \times \{0\}$).
...
\]
where the first factor $d$ is the per-entry degree bound of the indeterminate variable. The bound is invariant under the choice of which variable serves as $t$. The HNF cost is then $T_{1}^{\mathrm{HNF}}=\widetilde{O}(n^\omega K_1)$.
...
\end{enumerate}
With this method selection the first determinant never dominates: Kronecker$+$HNF controls it in the high-degree regime and cached expansion or sparse interpolation controls it in the high-variable regime. This matches the measured runtimes, where the final symbolic determinant (Step~4) consumes the overwhelming majority of the time. This is why the main text uses $T_{34}^{\mathrm{best}}$ as the overall symbolic term. The specialization to the well-determined case $k=1$, including the comparison of $T_{4}^{\mathrm{HNF}}$ with both $T_{1}^{\mathrm{sparse}}$ and $T_{1}^{\mathrm{minor}}$ used in Section~\ref{Section: Estimate}, is treated separately in Appendix~\ref{appendix:wellsq}.
...
\paragraph{Intermediate-minor support proxy.}
Several models (expansion, Bareiss) involve intermediate minors of $P$. Although a $j\times j$ minor is in general not a sub-polynomial of $\det(P)$, for systems satisfying our genericity assumption (Definition~\ref{def:generic}) every entry of $P$ has full support and no coefficient cancellation occurs in the determinantal expansion, so each intermediate minor's support is contained in the Minkowski sum of its row supports, which in turn is contained in $\operatorname{Supp}(\det P)$. Hence in the generic regime considered in this paper one may take $U \le S$ throughout, which is the convention used in our complexity estimates.
...
For $r$ rounds, we introduce intermediate variables $(w_{i,1}, w_{i,2})$ and $(z_{i,1}, z_{i,2})$ at round $i$ $(2 \leq i \leq r)$.
