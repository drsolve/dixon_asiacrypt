...
\begin{proof}
	...
	Attainment for generic systems follows from a Zariski-density argument. For $\mathcal{F}^*$ the row and column dimensions of $\mathcal{M}$ both equal the iterated-simplex monomial count $N^*$, which by the first part of the proof is the generic maximum. The condition that every iterated-simplex monomial appears in $\Delta$ with a nonzero coefficient is a finite conjunction of nonvanishing polynomial conditions on the coefficients of $\mathcal{F}$, all satisfied at $\mathcal{F}^*$; it therefore defines a non-empty, hence dense, Zariski-open subset of the irreducible coefficient space, on which the row and column dimensions of $\mathcal{M}$ equal $N^*$. Hence $D(\mathcal{F}) = N^*$ is attained generically. All multinomial coefficients arising in the relevant expansions have upper index at most $\sum_{i=2}^{n}(d_i-1)$, so the stated characteristic assumption prevents their vanishing modulo $\operatorname{char}(\mathbb{K})$.
...
\begin{proof}
	...
	\begin{equation}\notag
		\alpha_{n-j+1} + \sum_{\ell=n-j+2}^{n-1} \alpha_\ell' \leq \sum_{i=1}^{j-1} d_{n-i+1}
	\end{equation}
	(with the convention that the sums $\sum_{\ell=n-j+2}^{n-1}\alpha_\ell'$ and $\sum_{i=1}^{j-1}d_{n-i+1}$ are empty---and therefore equal to $0$---when $j=1$), set
	\begin{equation}\notag
		\alpha_{n-j+1}' := \alpha_{n-j+1}, \quad \alpha_{n-j+1}'' := 0.
	\end{equation}
	Otherwise, set
	\begin{equation}\notag
		\alpha_{n-j+1}' := \sum_{i=1}^{j-1} d_{n-i+1} - \sum_{\ell=n-j+2}^{n-1} \alpha_\ell', \quad 
		\alpha_{n-j+1}'' := \alpha_{n-j+1} - \alpha_{n-j+1}'.
	\end{equation}
	
	In both cases,
	\begin{equation}\notag
		\sum_{\ell=n-j+1}^{n-1} \alpha_\ell' \leq \sum_{i=1}^{j-1} d_{n-i+1},
	\end{equation}
	which establishes $(\alpha_1', \ldots, \alpha_{n-1}', 0) \in Q_{n-1}$.
	
	To verify $(\alpha_1'', \ldots, \alpha_n'') \in P_n$, we need $\sum_{\ell=1}^{n} \alpha_\ell'' \leq d_n$. This is immediate in the first case. In the second case:
	\begin{align}\notag
		\sum_{\ell=n-j+1}^{n} \alpha_\ell'' 
		&= \alpha_{n-j+1} - \left(\sum_{i=1}^{j-1} d_{n-i+1} - \sum_{\ell=n-j+2}^{n-1} \alpha_\ell'\right) + \sum_{\ell=n-j+2}^{n} \alpha_\ell'' \\[0.3em]\notag
		&= \sum_{\ell=n-j+1}^{n} \alpha_\ell - \sum_{i=1}^{j-1} d_{n-i+1} 
		\leq \sum_{i=1}^{j} d_{n-i+1} - \sum_{i=1}^{j-1} d_{n-i+1} 
		= d_n,
	\end{align}
