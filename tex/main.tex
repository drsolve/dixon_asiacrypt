...
\begin{enumerate}
	...
	\item We design three elimination paradigms for AO-induced systems: direct, iterative, and hybrid reduction. We illustrate them on \poseidon, \vision, and \texttt{XHash12}, respectively, providing concrete algebraic complexity estimates.
\end{enumerate}
...
Throughout this section, $D(\mathcal{F})$ denotes the size of the Dixon matrix $\mathcal{M}$ associated with $\mathcal{F}$, that is, the number of distinct $\xvec$-monomials (equivalently, of $\overline{\xvec}$-monomials) occurring in the Dixon polynomial. We denote a system of $n$ polynomials with total-degree bounds $d_1, d_2, \ldots, d_n$ as a $(d_1,\ldots,d_n)$-system. The special case $d_1=\cdots=d_n=d$ is referred to as an $(n;d)$-system, following~\cite{DBLP:conf/stoc/KapurS96}.
...
\begin{table}[h]
	...
	\begin{tabular}{lcc}
		...
		\midrule
		FGLM~\cite{DBLP:journals/jsc/FaugereGLM93,DBLP:journals/tosc/KoschatkoLR24} & $\displaystyle \mathcal{O}\left(n \cdot d_{\mathcal{I}}^{\omega}\right)$ & $\mathcal{O}\!\left(e^{-n\omega}\right)$ \\
		\bottomrule
	\end{tabular}
	\vspace{0.5em}
\end{table}
...
\emph{(P2) Successive elimination.} Reduction only lowers the exponents $\deg_{z_i}$ below $\alpha$; it does not by itself produce an element of $\mathbb{K}[\mathcal{X}]$. The remaining powers of the $z_i$ are removed by successive resultants, each followed by a reduction as in~\cite[Proposition~7]{DBLP:conf/crypto/BariantBBHOR25}, with the same substitution of $B_k(\cdot)$ for the coefficient count. Writing $D_i \le \alpha^{\,n-i}D_0$ for the weighted degree after the $i$-th elimination, and summing over all stages and all $k$ constraints,
\begin{equation}
...
\]
which shows an exponential decrease as $n$ increases. The $e^{-n\omega}$ form in Table~\ref{tab:complexity_formulas} is the limiting $d\to\infty$ asymptotic; for finite $d$ (which is the practical regime for AO primitives), the base is $((d-1)/d)^{d-1} > e^{-1}$, so the actual ratio is somewhat larger than $e^{-n\omega}$ but still exponentially small in $n$.
