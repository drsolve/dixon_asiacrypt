...
For these parameters the final merge~\eqref{eq:mitm_complexity} dominates every forward and backward stage of the chain, so it alone determines the overall complexity; the stage-by-stage figures, together with a rank bound that keeps the intermediate rungs below the merge, are collected in Appendix~\ref{appendix:vision}. The special structure of \vision yields observed degree growth far below the B\'ezout-based upper bounds. A more precise estimate is left for future work.
...
\end{equation}
The degree bounds and multivariate cost estimates are proved in Appendix~\ref{appendix:hybrid_cost_proof}.
