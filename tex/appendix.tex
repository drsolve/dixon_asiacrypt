...
\begin{proof}
	...
	\end{equation}
	Here $\sum_{i=2}^{j} d_{n-i+1}=\sum_{k=n-j+1}^{n-1} d_k$ is precisely the budget that $Q_{n-1}$ allows for the suffix starting at index $n-j+1$; note also that this budget increases by $d_{n-j+1}\ge 0$ from step $j-1$ to step $j$, so $\alpha_{n-j+1}'\ge 0$ in the second case. At $j=1$ the budget is the empty sum $0$, so the step forces $\alpha_n'=0$, as required by the shape of the decomposition.
...
\[
\mathcal{T}_{\mathrm{Dixon}} = \mathcal{O}\left( nd \cdot \left( \frac{1}{n(d-1)+1} \binom{nd}{n} \right)^\omega \right).
\]
...
The complexity is:
\[
\mathcal{T}_{\mathrm{GB}} = \mathcal{O}\left(\binom{n + d_{\text{reg}}}{d_{\text{reg}}}^\omega\right).
\]

With $d_{\text{reg}} = n(d-1)+1$, this becomes:
\[
\mathcal{T}_{\mathrm{GB}} = \mathcal{O}\left(\binom{n + n(d-1)+1}{n(d-1)+1}^\omega\right) = \mathcal{O}\left(\binom{nd+1}{n}^\omega\right).
\]
...
\begin{align*}
	\frac{\mathcal{T}_{\mathrm{GB}}}{\mathcal{T}_{\mathrm{Dixon}}} 
...
The complexity is:
\[
\mathcal{T}_{\mathrm{F5}} = \mathcal{O}\left(n \cdot d_{\text{reg}} \cdot \binom{n + d_{\text{reg}} - 1}{d_{\text{reg}}}^\omega\right).
\]

With $d_{\text{reg}} = n(d-1)+1$, this becomes:
\begin{align*}
	\mathcal{T}_{\mathrm{F5}} &= \mathcal{O}\left(n \cdot (n(d-1)+1) \cdot \binom{n + n(d-1)+1 - 1}{n(d-1)+1}^\omega\right) \\
...
the ratio becomes:
\begin{align*}
	\frac{\mathcal{T}_{\mathrm{F5}}}{\mathcal{T}_{\mathrm{Dixon}}} 
...
\begin{align*}
	\frac{\mathcal{T}_{\mathrm{F5P}}}{\mathcal{T}_{\mathrm{Dixon}}} 
...
The complexity is:
\[
\mathcal{T}_{\mathrm{FGLM}} = \mathcal{O}(n \cdot d_{\mathcal{I}}^\omega) = \mathcal{O}(n \cdot (d^n)^\omega) = \mathcal{O}(n \cdot d^{n\omega}).
\]
...
the ratio is:
\begin{align*}
	\frac{\mathcal{T}_{\mathrm{FGLM}}}{\mathcal{T}_{\mathrm{Dixon}}} 
...
Thus
\[
\frac{\mathcal{T}_{\mathrm{FGLM}}}{\mathcal{T}_{\mathrm{Dixon}}} = \mathcal{O}_d\!\left(n^{3\omega/2}\eta_d^{\,n\omega}\right),
\]
which shows an exponential decrease as $n$ increases for every fixed $d>1$. Moreover, $\eta_d\to e^{-1}$ as $d\to\infty$; for finite $d$ (which is the practical regime for AO primitives), $\eta_d>e^{-1}$.
...
\]
Using $\mathcal{T}_{\mathrm{Dixon}}=\Theta\bigl(d\,n^{1-3\omega/2}\gamma_d^{\,n\omega}\bigr)$ with $\gamma_d=d^{d}/(d-1)^{d-1}$, which follows from~\eqref{eq:fc_stirling}, the ratio is
\[
\frac{\mathcal{T}_{\mathrm{BNS}}}{\mathcal{T}_{\mathrm{Dixon}}}
=\widetilde{\Theta}\!\left(n^{\omega-1/2}d^{-\omega}\eta_d^{\,n\omega}\right)
=\widetilde{\mathcal{O}}_d\!\left(n^{\omega-1/2}\eta_d^{\,n\omega}\right),
\]
...
% \begin{align*}
	%     \frac{\mathcal{T}_{\mathrm{Six}}}{\mathcal{T}_{\mathrm{Dixon}}} &\approx \frac{\left( \frac{n}{n(d-1)+1} \binom{nd}{n} \right)^{\omega-1} \cdot n \binom{nd-d}{n-1}}{nd \left( \frac{1}{n(d-1)+1} \binom{nd}{n} \right)^\omega} = \frac{n^\omega \binom{nd-d}{n-1} (n(d-1)+1)}{nd \binom{nd}{n}} \\
...
\]
where the first factor $d$ is the per-entry degree bound of the indeterminate variable, and the second inequality uses $d\le b_\ell+1$, which holds since $b_\ell$ bounds a degree of the whole determinant while $d$ bounds the corresponding degree of a single entry. The bound is invariant under the choice of which variable serves as $t$. The HNF cost is then $T_{1}^{\mathrm{HNF}}=\widetilde{O}(n^\omega \delta_1)$.
...
\begin{table}[h]
	...
	\begin{tabular}{p{0.18\textwidth}p{0.34\textwidth}p{0.34\textwidth}}
		...
		Kronecker & $\widetilde{O}(n^\omega \delta_1)$ & $\widetilde{O}(M^\omega\delta_4)$ \\
...
\paragraph{Notation.}
Let $P(\yvec)$ be an $m\times m$ polynomial matrix in variables $\yvec=(y_1,\ldots,y_v)$. Let $b_i$ bound the degree of $\det(P)$ in $y_i$ (this determines the Kronecker substitution base, which must avoid overflow), and let $e_i\le b_i$ bound the degree of a single entry of $P$ in $y_i$. Define
\[
\delta = e_v \prod_{i=1}^{v-1}(b_i+1),
\qquad
N = \prod_{i=1}^{v}(b_i+1),
\]
where $\delta$ bounds the entry degree, hence the average column degree, of the univariate matrix obtained after Kronecker substitution.
...
For the first determinant we substitute $m=n$, $(\delta,N,S,L)=(\delta_1,N_1,S_1,L_1)$, and use $U=S_1$ as the intermediate-support proxy. For the final determinant we substitute $m=M$, $(\delta,N,S,L)=(\delta_4,N_4,S_4,L_4)$, and use $U=S_4$. This is exactly the convention used in the comparison plots and in the complexity-reporting code.
