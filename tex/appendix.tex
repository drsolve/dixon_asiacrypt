...
\]
Since $\omega>2$, we have $\omega(n-1)+1>2n-1$ for every $n\ge 2$, so $T_{4}^{\mathrm{HNF}}$ grows strictly faster than $T_{1}^{\mathrm{HNF}}$ as $d$ increases. For the borderline case $\omega=2$, the two exponents coincide ($\omega(n-1)+1 = 2n-1$), so the two stages have the same asymptotic rate in $d$ and their ordering is decided by the implied constants alone. These constants are themselves strongly $n$-dependent (they contain factors such as $n^{n-1}/n!$ inside $C_n^{(d)}=\Theta(d^{\,n-1})$ and $\prod_i(b_i+1)/d^{2n-2}$ inside $\delta_1=\Theta(d^{2n-1})$), and the present comparison is carried out at fixed $n$; no ordering of the two stages should therefore be read off from the powers of $n$ in this regime. The strict, regime-independent dominance of Step~4 is therefore an $\omega>2$ phenomenon, which is the practically relevant case: it is driven by the larger $C_n^{(d)}$-exponent ($C_n^{(d)}$ being polynomially large in both $n$ and $d$), and it is consistent with the measured running times, where the final symbolic determinant is the bottleneck. We accordingly state the dominance for $\omega>2$ and treat $\omega=2$ as an idealized boundary used only in the uniform-exponent security plots.
...
\end{enumerate}
For $\omega>2$, with this method selection the first determinant never dominates: Kronecker$+$HNF controls it in the high-degree regime and cached expansion or sparse interpolation controls it in the high-variable regime. The dominance is however asymptotic: at moderate $(n,d)$ the polynomial prefactors of the Step-1 models can still exceed the $\bigl(C_n^{(d)}\bigr)^{\omega-2}$ gap, and at $\omega=2$ the conclusion is reversed altogether, since the $\Theta\!\left(\bigl(C_n^{(d)}\bigr)^2\right)$ coefficient positions of the Dixon polynomial already match the $C_n^{(d)}$-exponent of Step~4. This is why the $\omega=2.37$ panel of Figure~\ref{fig:complexity} reports the maximum over the two stages. This matches the measured runtimes, where the final symbolic determinant (Step~4) consumes the overwhelming majority of the time. This is why the main text uses $T_{4}^{\mathrm{best}}$ as the overall symbolic term. The specialization to the well-determined case $k=1$, including the comparison of $T_{4}^{\mathrm{HNF}}$ with both $T_{1}^{\mathrm{sparse}}$ and $T_{1}^{\mathrm{minor}}$ used in Section~\ref{Section: Estimate}, is treated separately in Appendix~\ref{appendix:wellsq}.
