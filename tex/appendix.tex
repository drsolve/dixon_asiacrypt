...
\begin{align*}
	...
	&= \mathcal{O}_d\!\left(n^{3\omega/2}
	\left(\frac{d-1}{d}\right)^{(d-1)n\omega}\right),
\end{align*}
where $\mathcal{O}_d(\cdot)$ suppresses factors depending only on the fixed degree $d$.

Thus
\[
\frac{\mathcal{T}_{\text{FGLM}}}{\mathcal{T}_{\text{Dixon}}} = \mathcal{O}_d\!\left(n^{3\omega/2}\rho_d^{\,n\omega}\right),
\]
which shows an exponential decrease as $n$ increases for every fixed $d>1$. Moreover, $\rho_d\to e^{-1}$ as $d\to\infty$; for finite $d$ (which is the practical regime for AO primitives), $\rho_d>e^{-1}$.

\paragraph{BNS change of order~\cite{BerthomieuNeigerSafey2022}}

Under shape and stability assumptions, the change of order can be performed by computing the Hermite normal form of a $t\times t$ univariate polynomial matrix of average column degree $d_{\mathcal{I}}/t$, in $\widetilde{\mathcal{O}}(t^{\omega-1}d_{\mathcal{I}})$ operations, where $t$ is the number of elements of the grevlex basis whose leading monomial is divisible by the smallest variable. For generic $(n;d)$-systems, \cite[Tbl.~1]{BerthomieuNeigerSafey2022} gives $d_{\mathcal{I}}=d^{\,n}$ and $t=d^{\,n-1}/\sqrt{n}$, whence
\[
\mathcal{T}_{\mathrm{BNS}}
=\widetilde{\mathcal{O}}\!\left(\bigl(d^{\,n-1}n^{-1/2}\bigr)^{\omega-1}d^{\,n}\right)
=\widetilde{\mathcal{O}}\!\left(n^{-(\omega-1)/2}\,d^{\,n\omega-\omega+1}\right).
\]
Using $\mathcal{T}_{\text{Dixon}}=\Theta\bigl(d\,n^{1-3\omega/2}\gamma_d^{\,n\omega}\bigr)$ with $\gamma_d=d^{d}/(d-1)^{d-1}$, which follows from~\eqref{eq:fc_stirling}, the ratio is
\[
\frac{\mathcal{T}_{\mathrm{BNS}}}{\mathcal{T}_{\text{Dixon}}}
=\widetilde{\Theta}\!\left(n^{\omega-1/2}d^{-\omega}\rho_d^{\,n\omega}\right)
=\widetilde{\mathcal{O}}_d\!\left(n^{\omega-1/2}\rho_d^{\,n\omega}\right),
\]
i.e.\ the same exponentially decreasing factor as for FGLM, the two differing only by a polynomial factor in $(n,d)$. The gain of~\cite{BerthomieuNeigerSafey2022} over Sparse-FGLM is exponential in $n$, but against the $\widetilde{\mathcal{O}}(n\,d_{\mathcal{I}}^{\omega})$ estimate tabulated here it is only polynomial, so the qualitative comparison with Dixon is unchanged.

\paragraph{Other elimination algorithms.}
The FGLM estimate $\mathcal{O}(n\,d_{\mathcal{I}}^{\omega})$ used above does not include the construction of the multiplication matrices. For two eliminated variables, structured Sylvester-resultant algorithms such as Villard's~\cite{Villard2018} have a better asymptotic complexity, even though the Dixon construction works with a smaller matrix. On the AO systems of Section~\ref{Section:Application} the relevant Gr\"obner competitors are moreover the FreeLunch~\cite{DBLP:conf/crypto/BariantBLAOPR24} and CheapLunch~\cite{DBLP:journals/iacr/BakBBBOP25} attacks, for which the grevlex basis is essentially free and the change-of-order step dominates; Dixon is then attractive in the few-variable/high-degree regime, while CheapLunch is expected to be preferable as the number of variables grows.
...
\begin{figure}[htbp]
	...
	\begin{subfigure}{\textwidth}
		\centering
		\includegraphics[width=0.85\textwidth]{complexity_vs_degree_n5w237.png}
	\end{subfigure}
	
	\caption*{Figure~\ref{fig:complexity} (continued). The bottom panel uses $\omega=2.37$. Since the asymptotic dominance of the final determinant sets in only for larger $(n,d)$ than those plotted, the Dixon curve of this panel reports the maximum over the two symbolic stages, $\max\{T_{1}^{\mathrm{best}},T_{4}^{\mathrm{best}}\}$, which over this range is governed by Step~1.}
\end{figure}
...
\end{enumerate}
For $\omega>2$, with this method selection the first determinant never dominates: Kronecker$+$HNF controls it in the high-degree regime and cached expansion or sparse interpolation controls it in the high-variable regime. The dominance is however asymptotic: at moderate $(n,d)$ the polynomial prefactors of the Step-1 models can still exceed the $M^{\omega-2}$ gap, and at $\omega=2$ the conclusion is reversed altogether, since the $\Theta(M^2)$ coefficient positions of the Dixon polynomial already match the $M$-exponent of Step~4. This is why the $\omega=2.37$ panel of Figure~\ref{fig:complexity} reports the maximum over the two stages. This matches the measured runtimes, where the final symbolic determinant (Step~4) consumes the overwhelming majority of the time. This is why the main text uses $T_{4}^{\mathrm{best}}$ as the overall symbolic term. The specialization to the well-determined case $k=1$, including the comparison of $T_{4}^{\mathrm{HNF}}$ with both $T_{1}^{\mathrm{sparse}}$ and $T_{1}^{\mathrm{minor}}$ used in Section~\ref{Section: Estimate}, is treated separately in Appendix~\ref{appendix:wellsq}.
...
At the idealized boundary $\omega=2$, the $M$-exponents of $T_{4}^{\mathrm{HNF}}$ and of both $T_{1}^{\mathrm{sparse}}$ and $T_{1}^{\mathrm{minor}}$ coincide, so the asymptotic dominance is determined by the polynomial-in-$(n,d)$ prefactors rather than by an $M$-exponent gap. The value $\omega=2$ is used in this paper only for \vision, for consistency with~\cite{DBLP:conf/asiacrypt/LiuMM24}; there it is applied identically to Dixon and to every competitor, so the comparison should be read as a ``uniform-$\omega$ ratio'' rather than as a literal asymptotic statement about any single method. For \vision\ this is why the reported cost is the maximum over \emph{all} stages of the chain rather than the Step~4 term alone (Appendix~\ref{appendix:vision}). For any $\omega>2$ already at infinitesimal margin, the $M^{\omega-2}$ factor reactivates and $T_{4}^{\mathrm{HNF}}$ dominates $T_{1}^{\mathrm{sparse}}$; condition~\eqref{eq:minor_threshold} then governs the comparison with $T_{1}^{\mathrm{minor}}$. The Strassen exponent $\omega\approx 2.81$ used in the timing tables comfortably sits in this regime.
...
\paragraph{A rank bound for the ladder rungs.}
Each rung of the chain eliminates a block of two variables from the incoming relation $R$ together with two local equations. Unlike a generic three-polynomial system, only $R$ has a degree growing with the number of rounds, and this alone already forces the Dixon matrix to be strongly rank-deficient. The next lemma makes this precise; it requires neither genericity nor an assumption on $\operatorname{char}(\mathbb{K})$, so it applies verbatim over $\mathbb{F}_{2^\ell}$ (cf.\ Remark~\ref{rem:support_vs_rank}).

\begin{lemma}[Rung rank bound]\label{lem:rung_rank}
	Let $\xvec=(x_1,x_2)$ be the eliminated block, let $R\in\mathbb{K}[\xvec]$ have total degree at most $D$, and let $f_1,f_2$ have degree at most $e$ in $\xvec$. Let $\mathcal{M}$ be the Dixon matrix obtained by eliminating $\xvec$ from $\{R,f_1,f_2\}$. Then
	\begin{equation}\label{eq:rung_rank}
		\operatorname{rank}\mathcal{M}\;\le\;\frac{e(e+3)}{2}\,D+\frac{e(3e-1)}{2},
	\end{equation}
	and the Dixon polynomial $\Delta$ has $\mathcal{O}(e^4D^3)$ terms.
\end{lemma}

\begin{proof}
	Write $\Delta=X\mathcal{M}\overline{X}^{T}$ and recall the two elementary facts that a bilinear form $\sum_{a\le r}u_a(\xvec)v_a(\bar\xvec)$ has coefficient matrix of rank at most $r$, and that these ranks are subadditive under sums and submultiplicative under products. In the refined construction~\eqref{eq:Dixon_polynomial_improve} with $n=3$, the entry $\widehat{C}_{1,j}$ has $\bar\xvec$-degree $0$, the entry $\widehat{C}_{2,j}$ involves $\bar{x}_1$ only, and $\widehat{C}_{3,j}$ involves $x_2$ only on the $\xvec$-side; in rows $2$ and $3$ the difference quotient lowers the $\xvec$-degree by one.
	
	Expanding $\det\widehat{C}$ along the column of $R$ gives $\Delta=\sum_{i=1}^{3}(-1)^{i+1}\widehat{C}_{i,R}\,m_i$, where $m_i$ is the $2\times 2$ minor formed by the columns of $f_1,f_2$ with row $i$ deleted. The three factors attached to $R$ have ranks at most $1$, $D$ and $D$ respectively (a single $\bar\xvec$-monomial for $i=1$; at most $D$ powers of $\bar x_1$ for $i=2$; at most $D$ powers of $x_2$ for $i=3$). The minors are bounded by their $\bar\xvec$-supports: $m_3$ involves $\bar x_1$ alone with degree $\le e-1$, giving $e$ monomials; $m_2$ has $\bar\xvec$-degree $\le e-1$, giving $\binom{e+1}{2}$ monomials; and $m_1$ has $\bar\xvec$-support inside $\{\bar x_1^{a}\bar x_2^{b}: b\le e-1,\ a+b\le 2e-2\}$, of cardinality $e(3e-1)/2$. Combining the three products yields~\eqref{eq:rung_rank}. Finally, $\widehat{C}_{2,R}$ and $\widehat{C}_{3,R}$ each involve only three of the four variables, with degree below $D$ in each, hence have $\mathcal{O}(D^3)$ terms, while every $m_i$ has $\mathcal{O}(e^4)$ terms.
\end{proof}
...
Tables~\ref{tab:vision_stages_r4} and~\ref{tab:vision_stages_all} record the cost of every elimination carried out by Method~2, so that the claim of Section~\ref{subsec:method2} that the final merge dominates can be read off directly. For a rung we list the degree $d$ of the local block, the total degrees $D_{\mathrm{in}}$ and $D_{\mathrm{out}}$ of the incoming and outgoing relations, the support dimension $D(\mathcal{F})$ of the associated Dixon matrix, the bound~\eqref{eq:rung_rank} on the size $\rho$ of the maximal-rank submatrix extracted in Step~3, and the costs of Steps~1, 3 and~4; the cost of the rung is their maximum. Forward rungs are numbered $\mathrm{F}i$ and backward rungs $\mathrm{B}i$ according to their position in the ladder. All entries are $\log_2$ field operations at $\omega=2$. Step~1 uses the $\mathcal{O}(e^4D_{\mathrm{in}}^{3})$ term count of Lemma~\ref{lem:rung_rank}, which is far below the $\Theta(D(\mathcal{F})^2)$ coefficient positions of a dense Dixon polynomial. Since $\rho$ is well below $D(\mathcal{F})$, Step~3 is not run as a full elimination on $\mathcal{M}$: right-multiplying $\mathcal{M}$ by a random $D(\mathcal{F})\times c\rho$ matrix reveals the rank with high probability in $\mathcal{O}(D(\mathcal{F})\rho^{\omega-1})$ field operations, which is the figure reported below and reduces to~\eqref{eq:complexity_step3} when $\rho=D(\mathcal{F})$, as at the merge.
