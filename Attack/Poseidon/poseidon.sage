...
class Poseidon:
    def __init__(self, prime=2^64-2^8-1, R_F=8, R_P=42, t=24, alpha=3):
        F = GF(prime)
    ...
    def perm(self, input_words):
        ...
        # Use floor division for first rounds, remainder goes to end
        R_f_first = R_F // 2      # First full rounds
        R_f_last = R_F - R_f_first  # Last full rounds (handles odd R_F)
        ...
        if R_P > 0:
            ...
            for r in range(0, R_P):
                # Round constants, nonlinear layer, matrix multiplication
                #state_words = list(vector(state_words) * M_i)
                state_words[0] = (state_words[0])^self.alpha
    ...
    def perm_original(self, input_words):
        ...
        for r in range(R_F + R_P):
            ...
            if r < R_f_first or r >= (R_f_first + R_P):  # full round
                state_words = [state_words[i]^self.alpha for i in range(t)]
            else:  # partial round
                state_words[0] = state_words[0]^self.alpha
...
def poseidon_last_squeeze_poly_system(poseidon, xs, hash_digest):
    ...
    cap = t - rate
    system = [xs[i + rate] - (xs[i] + round_constants_field[0][i])^self.alpha for i in range(rate)] # ralating input and after-S-Box of first round # rate equations
    curr_state = [xs[i + rate] for i in range(rate)] + [round_constants_field[0][i]^self.alpha for i in range(rate, t)] # state just before first MDS matrix
...
def poseidon_last_squeeze_trace(poseidon, preimage):
    ...
    state = matrix([[el[0]^self.alpha for el in state.rows()]]).transpose()
    ...
    for r in range(1, num_rounds-1):
        ...
        if r < (R_F // 2) or r >= (R_F // 2) + R_P: # full round
            ...
            state[0,0] = state[0,0]^self.alpha
